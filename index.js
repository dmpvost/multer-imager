...
S3Storage.prototype._handleFile = function (req, file, cb) {
    ...
    self.options.filename(req, file, function (err, filename) {
        ...
        if (self.options.gm.custom === true) {
            gm(file.stream)
                .resize(self.options.gm.width * 1.4, self.options.gm.height * 1.4, "^")
                .gravity("Center")
                .quality(self.options.gm.quality)
                .extent(self.options.gm.width, self.options.gm.height)
                .stream(self.options.gm.format || DEFAULT_FORMAT)
                .pipe(outStream);
        } else {
            gm(file.stream)
                .resize(self.options.gm.width, self.options.gm.height, "^")
                .quality(self.options.gm.quality)
                .stream(self.options.gm.format || DEFAULT_FORMAT)
                .pipe(outStream);
        }
